...
function getBasicHttpOptions(http_options) {
  const apiKey = configuration.default_options.api_key;
...
function getPathWithIdentifier(path, params, actionPath) {
  ...
  return params ? path + pathParams + actPath : path;
}
...
exports.getPathWithData = function (path, params, actionPath, data) {
  const pathWithData = getPathWithIdentifier(path, params, actionPath);
  let dataParam = '';
...
function execute(method, path, data, http_options, cb) {
  ...
  request(http_options, (error, response, body) => {
